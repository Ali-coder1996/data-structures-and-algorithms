...
let biggerThanLuke = (arr) => {
  // Solution code here...
  const val = arr.reduce((acc, value) => {
    if (parseInt(value.mass) > parseInt(arr[0].mass)) {
      if (acc) {
        acc = acc +' - ';
      }acc = acc + value.name;}return acc;}, '');return val;
};
...
const detectTicTacToeWin = (board) => {
  ...
  for (let i = 0; i < 9; i + 3) {
    if (list[i] == list[i + 2] && list[i] == list[i + 1] && list[i] != '') {
      return true;}
       else {
      for (let i = 0; i < 3; i++) {
        if (list[i] == list[i + 6] && list[i] == list[i + 3] &&  list[i] != '') {
          return true;
        } else {
          for (let i = 0; i < 3; i + 2) {
            if (list[i] == list[8 - i] &&  list[i] == list[4] && list[i] != '') {
              return true;
            }
            else {
              return false; }}
